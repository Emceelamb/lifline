...
function mathLines(){
    for(let i = 0; i<allDrawingPts.length;i++){
        for(let j = 0; j<allDrawingPts[i].length;j++){
            allDrawingPts[i][j].x*=0.001;
            allDrawingPts[i][j].y*=0.5;

            // allDrawingPts[i][j].y*=0.001;
            console.log(allDrawingPts[i][j].x)
...
function drawAllLines() {
    ...
    for (var i = 0; i < allDrawingPts.length; i++) {
        ...
        for (var j = 0; j < allDrawingPts[i].length; j++) {
            stroke(i * 30)
            allDrawingPts[i][j].y = allDrawingPts[i][j].y * map(mouseY, 0, windowHeight, 0.5, 0.59);
            ...
            let r = map(map(allDrawingPts[i][j].y, 0, 1000, 0, 20), map(allDrawingPts[i][0].y, 0, 1000, 0, 20), map(allDrawingPts[i][allDrawingPts[i].length - 1].y, 0, 1000, 0, 20), 0 + rpos, 2 + rpos);
            let x = r * sin(allDrawingPts[i][j].x ) 
            let y = r * cos(allDrawingPts[i][j].x ) ;
            ...
            endShape();
            globalpos += 0.01;
            rpos += map(mouseX, 0, width, 0.01, 0.5);
